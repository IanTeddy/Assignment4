...
const getAstroData = () => {
    ...
    if (inputDate === "") return;
    const API_URL = `https://api.nasa.gov/planetary/apod?api_key=${API_KEY}&date=${queryInput}`;

    // fetching API Data, send a GET request and response is then converted to JSON format
    fetch(API_URL).then(response => response.json()).then(data => {
        // if data is empty 
        if (!data.length) return alert(`Request date might be out of date or invalid input`);
